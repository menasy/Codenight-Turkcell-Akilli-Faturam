...
function Login() {
	const { users, getUserWithPlan, loading, error } = useUser();
	const { login } = useApp();
	...
	const handleUserSelect = (user) => {
		const userWithPlan = getUserWithPlan(user.user_id);
		setSelectedUser(userWithPlan);
		setIsDropdownOpen(false);
		console.log("Seçilen kullanıcı:", userWithPlan);
	};
		...
		return (
			<div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center p-4">
				<div className="text-center">
					...
					<p className="text-white text-lg">Hata: {error}</p>
				</div>
			</div>
		);
	}
													...
													<div className="flex items-center space-x-4">
														...
														<div className="flex-1">
															...
															<div className="text-sm text-slate-500 group-hover:text-slate-600">
																{userWithPlan?.current_plan?.plan_name || 'Plan yükleniyor...'}
															</div>
														</div>
						...
						<div className="mt-6 p-4 bg-gradient-to-r from-slate-50 to-yellow-50 rounded-xl border border-yellow-200">
							<div className="flex items-center justify-between">
								...
								<span className="text-xs bg-yellow-200 text-yellow-800 px-2 py-1 rounded-full font-medium">
									{selectedUser.current_plan?.plan_name || 'Plan yükleniyor...'}
								</span>
							</div>
						</div>
					)}
